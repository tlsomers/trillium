...
Section collect.
  ...
  Proof.
    ...
    match goal with
    |- (λ x, (λ y, ?P) _) _ =>
      simpl; apply (map_fold_weak_ind (M := gmap _) (B := gset B) (λ y, λ x, P))
    end; [ done | exact ∅ | |].
    - intros. rewrite collect_singleton delete_empty collect_empty right_id_L //.
  ...
  Proof.
    ...
    match goal with
    |- (λ x, (λ y, ?P) _) _ =>
    simpl; apply (map_fold_weak_ind (M := gmap _) (λ y, λ x, P))
    end.
